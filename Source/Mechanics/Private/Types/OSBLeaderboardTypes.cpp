...
FColumnMetaDataBP FColumnMetaDataBP::FromNative(FColumnMetaData &Column)
{
   ...
   // Result.ColumnName = Column.ColumnName;
    Result.DataType = static_cast<EOnlineKeyValuePairDataType_>(Column.DataType);
    return Result;
}

FColumnMetaData FColumnMetaDataBP::ToNative() const
{
    return FColumnMetaData(this->ColumnName, static_cast<EOnlineKeyValuePairDataType::Type>(this->DataType));
}
...
{
    FName nukk;
    return nukk; //this->Leaderboard->LeaderboardName;
}
...
{
    FName Stat;
    return Stat; //this->Leaderboard->SortedColumn;
}
