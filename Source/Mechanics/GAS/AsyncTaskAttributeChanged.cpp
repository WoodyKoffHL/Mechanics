...
// Delegate function that is called when an attribute changes

void UAsyncTaskAttributeChanged::AttributeChanged(const FOnAttributeChangeData& Data)
{
	OnAttributeChanged.Broadcast(Data.Attribute, Data.NewValue, Data.OldValue);

	FTimerHandle RetriggibleDelayHandle;

	
	if (GameInstanceRef != nullptr)
	{
		// FTimerManager& TimerManager = GameInstanceRef->GetTimerManager();
	// TimerManager.SetTimer(RetriggibleDelayHandle,this,&UAsyncTaskAttributeChanged::PrintString,RetriggibleDelayTime,false);
	}
}

//Print String
void UAsyncTaskAttributeChanged::PrintString()
{
